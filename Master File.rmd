...
h2 <- ggplot(data = Fish2, aes(x = Length)) + 
  geom_histogram(binwidth = 2, 
                 fill = "cornflowerblue") + 
  labs(title = "Captured Fish Measured Length in mm", 
       x = "Length in mm") 
...
s1 <- scatterplot(Fish2$Length, Fish2$Weight, 
            ...
            smooth = FALSE,             
            regLine = FALSE,
            main = "Fish Length on Fish Weight Visualization", xlab = "Length in mm", ylab = "Weight in grams",
...
s3 <- ggplot(Fish2, aes(x=log(Length), y=log(Weight))) +
  ...
  labs(title = "Natural Log Measured Weight on Measured Length", 
       ...
       y = "Natural Log Weight in grams")
s3
lm1.out <- lm(log(Fish2$Length) ~ log(Fish2$Weight))
  summary(lm1.out)
```
...
plot(Fish2$Weight, Fish2$Length, xlab="Fish Weight (g)", pch=19,
     ylab="Fish 2 Length (mm)", cex.lab=1.25, cex.axis=1.25)
...
#### Part 4f)
```{r CI and PI} 
#paste code for lesson 8, still need to be refined
lm2.out <- lm(Fish2$Length ~ Fish2$Weight, data=data.frame(Fish2)) # for y given x, x=70
new.WT <- data.frame(Length = 70)
(CI.L70 <- predict(lm2.out, newdata = new.WT, interval = "confidence", level = 0.95))
(PI.L70 <- predict(lm2.out, newdata = new.WT, interval = "prediction", level = 0.95))
```
