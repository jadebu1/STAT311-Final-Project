...
h2 <- ggplot(data = Fish2, aes(x = Length)) + 
  geom_histogram(binwidth = 2, 
                 fill = "cornflowerblue") + 
  labs(title = "Captured Fish Measured Fork Length (mm)", 
       x = "Length (mm)") 
summary(Fish2$Length)
h3 <- ggplot(data = Fish2, aes(x = Weight)) +
  geom_histogram(binwidth = 2, 
                 fill = "cornflowerblue") + 
  labs(title = "Captured Fish Measured Weight (g)", 
       x = "Weight (g)") 
...
s1 <- scatterplot(Fish2$Length, Fish2$Weight, 
            ...
            smooth = FALSE,             
            regLine = FALSE,
            main = "Figure 2 - Fish Fork Length on Fish Weight Visualization", 
            xlab = "Fork Length (mm)", 
            ylab = "Weight in (g)",
            cex.axis = 1.2, 
            cex.lab = 1.2, 
...
s2 <- ggplot(Fish2, aes(x=log(Length), y=log(Weight))) +
  geom_point(color="cornflowerblue", 
             size = 1, 
             alpha=.5) +  
  labs(title = "Figure 3 - Natural Log Measured Weight on Measured Fork Length", 
...
s3 <- ggplot(Fish2, aes(x=log(Length), y=log(Weight))) +
  ...
  labs(title = "Figure 4 - Natural Log Measured Weight on Natural Log Measured Fork Length", 
       x = "Natural Log Fork Length (mm)", 
       y = "Natural Log Weight (g)")
s3
...
```{r Reg Diagnostics}
par(mfrow = c(2, 2))
plot(Fish2$Weight, Fish2$Length, xlab="Weight (g)", pch=19,
     ylab="Fork Length (mm)", cex.lab=1.25, cex.axis=1.25)
...
mtext("Figure 5 - Regression Diagnostics for Natural Log Model", side = 3, line = -1.5, outer = TRUE)
```
...
b1 <- ggplot(Fish2, aes(x = Site, y = Length, fill = Site)) + 
  geom_boxplot() + labs(title = "Figure 6 - Fish Fork Length (mm) by Site", 
       x = "Site Location", 
       y = "Fork Length (mm)")
b1
```
